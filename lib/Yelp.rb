...
attr_reader :client
  ...
  def find_restaurants_by_location_type_rating(location, cuisine, radius, rating_input)
    # binding.pry
    new_array = client.search(location, {radius_filter: radius, category_filter: cuisine, rating: rating_input})
    new_array.businesses.collect do |x|
  ...
  # def find_restaurant_address(location)
  #   new_array = client.search(location)
  #   new_array.businesses.collect do |x|
  #     "name => #{x.name}
  #      address => #{x.location.display_address}
  #      phone => #{x.phone}"
  ...
  def find_restaurant_by_cuisine(location, cuisine)
    ...
    new_array.businesses.collect.with_index(1) do |x, index|
      "#{x.name} => #{x.review_count} reviews
      #{x.location.display_address} ==>
      #{x.phone}"
      end
    end
  ...
  def call
    ...
    while input != "exit"
      ...
      when "ratings"
        ...
        puts find_restaurants_by_location_type_rating(location, cuisine, radius, rating_input)
